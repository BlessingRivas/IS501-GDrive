...
<head>
    ...
    <title>Compartido conmigo - Google Drive</title>
                ...
                <div class="content-div"  style="position:relative">                    
                    <div class="container content-header">
                        <div class="container" style="float: left;width: 50%;position: relative;">
                            <button type="button" class="btn btn-link" id="btn-folder"><b>Compartido conmigo</b></button>                            
                        </div>
                    ...
                    <div class="folders-div">
                        ...
                        <div class="row">
                            <?php
                                ...
                                $stmt = $conexion->prepare('SELECT COUNT(*) AS CANTIDAD_CARPETAS FROM TBL_COMPARTIDOS WHERE CODIGO_USUARIO = ?');
                                ...
                                if($result['CANTIDAD_CARPETAS'] == 0){
                                    ...
                                    $stmt = $conexion->prepare('SELECT A.NOMBRE,A.CODIGO_CARPETA FROM TBL_CARPETAS A INNER JOIN TBL_COMPARTIDOS B ON(A.CODIGO_CARPETA = B.CODIGO_CARPETA) WHERE B.CODIGO_USUARIO = ?');
                                    $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                    $stmt->execute();
                        ...
                        <div class="container section-label">Archivos</div>
                            <div class="row">
                                <?php
                                    ...
                                    $stmt = $conexion->prepare('SELECT COUNT(*) AS CANTIDAD_ARCHIVOS FROM TBL_COMPARTIDOS WHERE CODIGO_USUARIO = ?');
                                    $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                    $stmt->execute();
                                    ...
                                    if($result['CANTIDAD_ARCHIVOS'] == 0){
                                        ...
                                        $stmt = $conexion->prepare('SELECT A.CODIGO_ARCHIVO,A.NOMBRE,C.ICONO FROM TBL_ARCHIVOS  A INNER JOIN TBL_COMPARTIDOS B ON (A.CODIGO_ARCHIVO = B.CODIGO_ARCHIVO) INNER JOIN TBL_TIPOS_ARCHIVOS C ON (A.CODIGO_TIPO_ARCHIVO = C.CODIGO_TIPO_ARCHIVO) WHERE B.CODIGO_USUARIO = ?');
                                        $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                        $stmt->execute();
                                        while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
                                            echo '
