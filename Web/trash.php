...
<head>
    ...
    <title>Papelera - Google Drive</title>
                ...
                <div class="content-div">
                    <div class="container content-header">
                        <div class="container" style="float: left;width: 50%;position: relative;">
                            <button type="button" class="btn btn-link" id="btn-folder"><b>Papelera</b></button>                            
                        </div>
                    ...
                    <div class="folders-div">
                        <div class="container section-label">Carpetas eliminadas</div>
                        <div class="row">
                            <?php
                                $gestor = new Gestor();
                                $conexion = $gestor->getConexion();

                                $stmt = $conexion->prepare('SELECT COUNT(*) AS CANTIDAD_CARPETAS FROM TBL_CARPETAS WHERE CODIGO_USUARIO = ? AND ELIMINADO = 1');
                                ...
                                if($result['CANTIDAD_CARPETAS'] == 0){
                                    ...
                                    $stmt = $conexion->prepare('SELECT NOMBRE,CODIGO_CARPETA,FECHA_ELIMINADO FROM TBL_CARPETAS WHERE CODIGO_USUARIO = ? AND ELIMINADO = 1');
                                    $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                    $stmt->execute();
                                        ...
                                        <div class="col-12 col-md-4 col-lg-4">
                                            <div class="folder-card" ondblclick=getFolderFiles("';echo $result['CODIGO_CARPETA'].'")>
                                                <div class="folder-card-text">
                                                    <i style="font-size:2em;float:left;margin-top:5px;margin-right:5px" class="fas fa-folder"></i><span class="folder-card-name">'.$result['NOMBRE'].'</span><br>
                                                    <span class="folder-card-name">Eliminada:'.$result['FECHA_ELIMINADO'].'</span>
                                                </div>
                                                <span class="folder-undelete" title="Restaurar carpeta"><i class="fas fa-share" onclick=undeleteFolder("'; echo $result['CODIGO_CARPETA'].'")></i></span>
                                            </div>
                                        </div>
                                        ';
                                    }
                    ...
                    <div class="files-div">
                        ...
                        <div class="row">
                            <?php
                                ...
                                $stmt = $conexion->prepare('SELECT COUNT(*) AS CANTIDAD_ARCHIVOS FROM TBL_ARCHIVOS WHERE CODIGO_USUARIO = ? AND ELIMINADO = 1');
                                $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                $stmt->execute();
                                ...
                                if($result['CANTIDAD_ARCHIVOS'] == 0){
                                    ...
                                    $stmt = $conexion->prepare('SELECT A.CODIGO_ARCHIVO,A.NOMBRE,B.ICONO FROM TBL_ARCHIVOS  A INNER JOIN TBL_TIPOS_ARCHIVOS B ON (A.CODIGO_TIPO_ARCHIVO = B.CODIGO_TIPO_ARCHIVO) WHERE A.CODIGO_USUARIO = ? AND A.ELIMINADO = 1');
                                    $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                    $stmt->execute();
                                    while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
                                        echo '
                                        <div class="col-12 col-md-4 col-lg-3" id="file-card-'; echo $result['CODIGO_ARCHIVO'].'">
                                            <div class="file-card">
                                                ...
                                                <div class="file-bar">                                                    
                                                    <span class="file-undelete"><i class="fas fa-share" onclick=unDeleteFile("'; echo $result['CODIGO_ARCHIVO'].'")></i></span>
                                                </div>
                                            </div>
                                        </div>
                                        ';
