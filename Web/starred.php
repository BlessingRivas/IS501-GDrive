...
<head>
    ...
    <title>Destacados - Google Drive</title>
                ...
                <div class="content-div">
                    <div class="container content-header">
                        <div class="container" style="float: left;width: 50%;position: relative;">
                            <button type="button" class="btn btn-link" id="btn-folder"><b>Destacados</b></button>                            
                        </div>
                    ...
                    <div class="folders-div">
                        <div class="container section-label">Carpetas destacadas</div>
                        <div class="row">
                            <?php
                                $gestor = new Gestor();
                                $conexion = $gestor->getConexion();

                                $stmt = $conexion->prepare('SELECT COUNT(*) AS CANTIDAD_CARPETAS FROM TBL_DESTACADOS A INNER JOIN TBL_CARPETAS B ON (A.CODIGO_CARPETA = B.CODIGO_CARPETA) WHERE B.CODIGO_USUARIO = ?');
                                ...
                                if($result['CANTIDAD_CARPETAS'] == 0){
                                    echo '<span id="no-folders-found" style="color:lightslategray;margin-left:auto;margin-right:auto;font-size:1em;font-family:\'Open sans\';">¡No hay carpetas destacadas que mostrar!</span>';
                                } else {
                                    $stmt = $conexion->prepare('SELECT B.NOMBRE,A.CODIGO_CARPETA FROM TBL_DESTACADOS A INNER JOIN TBL_CARPETAS B ON (A.CODIGO_CARPETA = B.CODIGO_CARPETA) WHERE CODIGO_USUARIO = ?');
                                    $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                    $stmt->execute();
                    ...
                    <div class="files-div">
                        <div class="container section-label">Archivos Destacados</div>
                        <div class="row">
                            <?php
                                $gestor = new Gestor();
                                $conexion = $gestor->getConexion();

                                $stmt = $conexion->prepare('SELECT COUNT(*) AS CANTIDAD_ARCHIVOS FROM TBL_DESTACADOS A INNER JOIN TBL_ARCHIVOS B ON (A.CODIGO_ARCHIVO = B.CODIGO_ARCHIVO) WHERE B.CODIGO_USUARIO = ?');
                                $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                $stmt->execute();
                                ...
                                if($result['CANTIDAD_ARCHIVOS'] == 0){
                                    ...
                                    $stmt = $conexion->prepare('SELECT A.CODIGO_ARCHIVO,B.NOMBRE,C.ICONO FROM TBL_DESTACADOS A INNER JOIN TBL_ARCHIVOS  B ON (A.CODIGO_ARCHIVO = B.CODIGO_ARCHIVO) INNER JOIN TBL_TIPOS_ARCHIVOS C ON (B.CODIGO_TIPO_ARCHIVO = C.CODIGO_TIPO_ARCHIVO) WHERE B.CODIGO_USUARIO = ?');
                                    $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                    $stmt->execute();
                                    while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
                                        echo '
                                        <div class="col-12 col-md-4 col-lg-3" id="file-card-'; echo $result['CODIGO_ARCHIVO'].'">
                                            <div class="file-card">
                                                ...
                                                <div class="file-bar">
                                                    <span class="file-stare"><i class="fas fa-star" onclick=unStareFile("'; echo $result['CODIGO_ARCHIVO'].'")></i></span>
