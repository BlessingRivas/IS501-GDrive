...
<body style="overflow-x: hidden">
    ...
    <?php include('folder-info-div.php');?>
        <div class="row">
            <div class="col-2">
                <div class="container">
            ...
            <div class="col-10">
                <div class="content-div" style="position:relative">                                        
                    <div class="container content-header">
                        <div class="container" style="float: left;width: 50%;position: relative;">
                            <button type="button" class="btn btn-link" id="btn-folder">Mi unidad&nbsp;&nbsp;&nbsp;<i class="fas fa-angle-down"></i></button>
                            <div class="container menu-div folder-div">
                                <?php include('menu-nuevo.php');?>                                                                           
                            </div>                            
                        </div>
                    ...
                    <div class="folders-div">
                        ...
                        <div class="row">
                            <?php
                                ...
                                $stmt = $conexion->prepare('SELECT COUNT(*) AS CANTIDAD_CARPETAS FROM TBL_CARPETAS WHERE CODIGO_USUARIO = ? AND CODIGO_CARPETA_PADRE IS NULL');
                                ...
                                if($result['CANTIDAD_CARPETAS'] == 0){
                                    ...
                                    $stmt = $conexion->prepare('SELECT NOMBRE,CODIGO_CARPETA FROM TBL_CARPETAS WHERE CODIGO_USUARIO = ? AND CODIGO_CARPETA_PADRE IS NULL');
                                    $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                    $stmt->execute();
                                    while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
                                        ...
                                        <div class="col-12 col-md-4 col-lg-3">
                                            <div class="folder-card" ondblclick=getFolderFiles("';echo $result['CODIGO_CARPETA'].'")>
                                                ...
                                                <span class="folder-edit" title="Editar propiedades"><i class="fas fa-pen" onclick=getFolderInfo("'; echo $result['CODIGO_CARPETA'].'")></i></span>
                                            </div>
                                        </div>
                                        ';
                                    }
                    ...
                    <div class="files-div">
                        ...
                        <div class="row">
                            <?php
                                ...
                                $stmt = $conexion->prepare('SELECT COUNT(*) AS CANTIDAD_ARCHIVOS FROM TBL_ARCHIVOS WHERE CODIGO_USUARIO = ? AND ELIMINADO = 0 AND CODIGO_CARPETA IS NULL');
                                $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                $stmt->execute();
                                ...
                                if($result['CANTIDAD_ARCHIVOS'] == 0){
                                    echo '<span id="no-folders-found" style="color:lightslategray;margin-left:auto;margin-right:auto;font-size:1em;font-family:\'Open sans\';">¡No hay archivos que mostrar!</span>';
                                } else {
                                    $stmt = $conexion->prepare('SELECT A.CODIGO_ARCHIVO,A.NOMBRE,B.ICONO FROM TBL_ARCHIVOS  A INNER JOIN TBL_TIPOS_ARCHIVOS B ON (A.CODIGO_TIPO_ARCHIVO = B.CODIGO_TIPO_ARCHIVO) WHERE CODIGO_USUARIO = ? AND ELIMINADO = 0');
                                    $stmt->bindParam(1,$_SESSION['codigo_usuario']);
                                    $stmt->execute();
                                    while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
                                        echo '
                                        <div class="col-12 col-md-4 col-lg-3" id="file-card-'; echo $result['CODIGO_ARCHIVO'].'">
                                            <div class="file-card">
                                                ...
                                                <div class="file-bar">
                                                    <span title="Destacar archivo" class="file-stare"><i class="fas fa-star" onclick=stareFile("'; echo $result['CODIGO_ARCHIVO'].'")></i></span>
                                                    <span title="Compartir archivo" class="file-share"><i class="fas fa-share-alt" onclick=shareFile("'; echo $result['CODIGO_ARCHIVO'].'")></i></span>
                                                    <span title="Eliminar archivo" class="file-delete"><i class="fas fa-trash-alt" onclick=deleteFile("'; echo $result['CODIGO_ARCHIVO'].'")></i></span>
                                                    <span title="Edtar propiedades" class="file-edit"><i class="fas fa-pen" onclick=getFileInfo("'; echo $result['CODIGO_ARCHIVO'].'")></i></span>                                                
                                                </div>
                                            </div>
                                        </div>
                                        ';
